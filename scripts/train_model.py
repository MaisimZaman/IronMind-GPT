...
model = GPT(config)
=======
    return x.to(device), y.to(device)  # 🔥 THIS FIXES IT


# --- Initialize Model ---
model = GPT(config)
model = model.to(device)  # ✅ FIXED — ensures model is on same device as data

>>>>>>> 68d23eb22e5e8e943d1cb43361fcafed09045ff1
...
=======
            torch.save(model.state_dict(), os.path.join(config.out_dir, 'ckpt.pt'))  # Local
            torch.save(model.state_dict(), '/content/drive/MyDrive/codewriter_model.pt')  # Permanent
>>>>>>> 68d23eb22e5e8e943d1cb43361fcafed09045ff1
